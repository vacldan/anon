# -*- coding: utf-8 -*-
"""
Perfect Czech Document Anonymizer
=================================
A perfect anonymization system that correctly handles Czech names in different cases.

Usage:
    python anonymizer_perfect.py input.docx [--output output.docx] [--level full]
"""
...
class PerfectCzechNameDetector:
    """Perfect Czech name detection with case handling"""
    ...
    def get_base_form(self, name: str) -> str:
        """Get base form of a first name (nominative case)"""
        normalized = self.normalize_name(name)
        
        # Handle common Czech case endings for first names
        case_endings = [
            ('a', ''),  # Jana -> Jan
            ('y', ''),  # Jany -> Jan
            ('e', ''),  # Jane -> Jan
        ...
        for ending, replacement in case_endings:
    ...
    def get_surname_base_form(self, surname: str) -> str:
        ...
        # Try to find base form
        for ending, replacement in surname_endings:
            if normalized.endswith(ending):
                base = normalized[:-len(ending)] + replacement
...
class PerfectAnonymizer:
    """Perfect anonymizer with proper case handling"""
    
    def __init__(self, level: AnonymizationLevel = AnonymizationLevel.STANDARD):
        self.level = level
        self.name_detector = PerfectCzechNameDetector()
    ...
    def anonymize_names(self, text: str) -> str:
        """Anonymize names with proper case handling"""
        ...
        while i < len(potential_names):
            ...
            if (i + 1 < len(potential_names) and 
                ...
                # Single name - check if it matches existing person
                key = self.name_detector.get_single_name_key(word)
                if key != ("", ""):
                    # Check if this name belongs to an existing person
                    found_existing = False
                    for existing_key, existing_tag in self.person_mappings.items():
                        if (key[0] and existing_key[0] and key[0] == existing_key[0]) or \
                           (key[1] and existing_key[1] and key[1] == existing_key[1]):
                            # Found matching person
                            self._add_replacement(existing_tag, word)
                            result_text = self._replace_text(result_text, start, end, existing_tag, word)
...
class DocumentProcessor:
    ...
    def __init__(self, anonymizer: PerfectAnonymizer):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Perfect Czech Document Anonymizer with Case Handling',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python anonymizer_perfect.py document.docx
  python anonymizer_perfect.py document.txt --level full --output anonymized.txt
  python anonymizer_perfect.py data.docx --level standard --log-level DEBUG
        """
    )
    ...
    try:
        ...
        print(f"🔧 Inicializace perfektního anonymizátoru s úrovní: {level.value}")
        
        anonymizer = PerfectAnonymizer(level)
