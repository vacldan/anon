# -*- coding: utf-8 -*-
"""
Working Czech Document Anonymizer
=================================
A simple and reliable anonymization system for Czech documents.

Usage:
    python anonymizer_working.py input.docx [--output output.docx] [--level full]
"""
...
class WorkingAnonymizer:
    """Working anonymizer with simple and reliable detection"""
    ...
    def anonymize_names(self, text: str) -> str:
        """Anonymize names using simple and reliable detection"""
        ...
        while i < len(potential_names):
            ...
            if (i + 1 < len(potential_names) and 
                ...
                # Single name - only surnames or isolated first names
                if self.name_detector.is_likely_surname(cleaned):
                    key = (cleaned.lower(), "")
                    if key not in self.person_mappings:
                        tag = self._new_tag("PERSON")
                        self.person_mappings[key] = tag
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Working Czech Document Anonymizer',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python anonymizer_working.py document.docx
  python anonymizer_working.py document.txt --level full --output anonymized.txt
  python anonymizer_working.py data.docx --level standard --log-level DEBUG
        """
    )
