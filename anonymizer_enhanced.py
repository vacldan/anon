...
sensitive personal data with anonymized markers while maintaining a detailed mapping.

Features:
- Advanced name detection using multiple strategies
- Comprehensive pattern matching for Czech personal data
...
Installation:
    pip install python-docx stanza unicodedata2

Usage:
    python anonymizer_enhanced.py input.docx [--output output.docx] [--level full]
"""
...
class CzechNameDetector:
    """Enhanced Czech name detection with multiple strategies"""
    
    def __init__(self):
        self.male_names = self._load_czech_male_names()
        self.female_names = self._load_czech_female_names()
        self.surnames = self._load_czech_surnames()
        ...
        # Possessive endings
        self.possessive_endings = {
            'ův', 'in', 'ova', 'ovo', 'ovi', 'ovy', 'ových', 'ovou', 'ové', 'ov'
        }
    
    def _load_czech_male_names(self) -> Set[str]:
        """Load common Czech male names"""
        return {
    ...
    def _load_czech_female_names(self) -> Set[str]:
        """Load common Czech female names"""
        return {
            'marie', 'jana', 'eva', 'hana', 'anna', 'věra', 'alena', 'lenka',
            'kateřina', 'lucie', 'petra', 'jana', 'zuzana', 'iveta', 'monika',
            'veronika', 'tereza', 'barbora', 'adéla', 'karolína', 'kristýna',
            'nikola', 'natálie', 'eliska', 'sophie', 'emma', 'olivia', 'amélie'
        }
    
    def _load_czech_surnames(self) -> Set[str]:
    ...
    def _is_name_variant(self, word: str) -> bool:
        ...
        # Simple heuristic for Czech name variants
        if len(word) < 3:
            return False
    ...
    def _is_surname_variant(self, word: str) -> bool:
        ...
        for ending in self.possessive_endings:
...
class PatternDetector:
    ...
    def _initialize_patterns(self) -> List[DetectionPattern]:
        ...
        patterns.append(DetectionPattern(
            name="birth_date",
            pattern=re.compile(r'\b\d{1,2}\.\s*\d{1,2}\.\s*\d{4}\b'),
            category="DATE"
        ))
        ...
        patterns.append(DetectionPattern(
            name="birth_number",
            pattern=re.compile(r'\b\d{2}[0156]\d{3,4}/?\d{4}\b'),
            category="BIRTH_ID",
            validation_func=self._validate_birth_number
        ))
        ...
        patterns.append(DetectionPattern(
            name="bank_account",
            pattern=re.compile(r'\b(?:\d{1,6}-)?\d{1,10}/\d{4}\b'),
            category="BANK"
        ))
        ...
        patterns.append(DetectionPattern(
            name="iban",
            pattern=re.compile(r'\bCZ\d{2}(?:\s?\d){20}\b', re.IGNORECASE),
            category="BANK"
        ))
        ...
        patterns.append(DetectionPattern(
            name="phone",
            pattern=re.compile(r'(?:\+?420[\s\-]?)?(?<!\d)(?:\d{3}[\s\-]?){2}\d{3}(?!\d)'),
            category="PHONE"
        ))
        ...
        patterns.append(DetectionPattern(
            name="vin",
            pattern=re.compile(r'\b(?![IOQ])[A-HJ-NPR-Z0-9]{17}\b'),
            category="VIN"
        ))
        
        # License plates (more specific pattern)
        patterns.append(DetectionPattern(
            name="license_plate",
            pattern=re.compile(r'\b[A-Z]{1,3}\s?[0-9]{1,4}[A-Z]?\b'),
            category="PLATE"
        ))
        ...
        if self.level == AnonymizationLevel.FULL:
            ...
            patterns.extend(self._get_additional_patterns())
        
        return patterns
    
    def _get_additional_patterns(self) -> List[DetectionPattern]:
        """Get additional patterns for full anonymization level"""
        return [
            # Social security numbers
            DetectionPattern(
                name="social_security",
                pattern=re.compile(r'\b\d{3}\s?\d{2}\s?\d{3}\b'),
    ...
    def _validate_birth_number(self, text: str) -> bool:
        """Validate Czech birth number (RČ)"""
        ...
        if len(clean) == 10:
            try:
                return int(clean) % 11 == 0 or int(clean) % 11 == 10
            except:
                return False
        return True
        ...
        for pattern in self.patterns:
            for match in pattern.pattern.finditer(text):
                ...
                if pattern.context_required:
                    ...
                    if not self._has_relevant_context(context, pattern.name):
                        continue
                
                # Validate if validation function exists
                if pattern.validation_func and not pattern.validation_func(match.group()):
                    continue
                ...
                results.append((pattern.category, match.group(), match.start(), match.end()))
        
        return results
    
    def _has_relevant_context(self, context: str, pattern_name: str) -> bool:
        ...
        keywords = context_keywords.get(pattern_name, [])
...
class EnhancedAnonymizer:
    """Enhanced anonymizer with comprehensive detection and mapping"""
    ...
    def __init__(self, level: AnonymizationLevel = AnonymizationLevel.STANDARD):
        ...
        self.stanza_pipe = self._initialize_stanza()
    
    def _initialize_stanza(self) -> Optional[Any]:
        """Initialize Stanza NLP pipeline if available"""
        try:
            import stanza
            self.logger.info("Initializing Stanza NLP pipeline...")
            nlp = stanza.Pipeline(
                lang="cs",
                processors="tokenize,mwt,pos,lemma",
                model_dir="data/models/stanza_cs",
                download_method=None,
                use_gpu=False
            )
            return nlp
        except ImportError:
            self.logger.warning("Stanza not available. Name detection will use heuristics only.")
            return None
        except Exception as e:
            self.logger.warning(f"Failed to initialize Stanza: {e}")
            return None
    ...
    def anonymize_names(self, text: str) -> str:
        """Anonymize names using multiple detection strategies"""
        if not self.stanza_pipe:
            return self._anonymize_names_heuristic(text)
        
        try:
            doc = self.stanza_pipe(text)
            return self._anonymize_names_stanza(text, doc)
        except Exception as e:
            self.logger.warning(f"Stanza processing failed: {e}. Falling back to heuristics.")
            return self._anonymize_names_heuristic(text)
    
    def _anonymize_names_stanza(self, text: str, doc: Any) -> str:
        """Anonymize names using Stanza analysis"""
        # This is a simplified version - the full implementation would be more complex
        # For now, fall back to heuristic method
        return self._anonymize_names_heuristic(text)
    
    def _anonymize_names_heuristic(self, text: str) -> str:
        """Anonymize names using heuristic detection"""
        # Use regex to find potential names in context
        ...
        def replace_name(match):
            ...
            if (self.name_detector.is_likely_first_name(cleaned) or 
                ...
                # Check for name pairs in nearby context
                start, end = match.span()
                context = text[max(0, start-50):min(len(text), end+50)]
                
                # Look for potential surname after this word
                next_words = text[end:end+50].split()[:3]  # Next 3 words
                for next_word in next_words:
                    next_cleaned = re.sub(r'[^\w]', '', next_word)
                    if self.name_detector.is_likely_surname(next_cleaned):
                        # Found name pair
                        key = (normalize_lower(cleaned), normalize_lower(next_cleaned))
    ...
    def anonymize_patterns(self, text: str) -> str:
        ...
        # Track already processed positions to avoid conflicts
        processed_ranges = []
...
class DocumentProcessor:
    ...
    def __init__(self, anonymizer: EnhancedAnonymizer):
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Enhanced Czech Document Anonymizer')
