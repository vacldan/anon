# -*- coding: utf-8 -*-
"""
Fixed Czech Document Anonymizer
===============================
A corrected and working anonymization system for Czech documents.

Usage:
    python anonymizer_fixed.py input.docx [--output output.docx] [--level full]
"""
...
class PatternDetector:
    ...
    def _initialize_patterns(self) -> List[Tuple[str, str, bool]]:
        ...
        # IČO (Czech company ID) - 8 digits with context
        patterns.append((r'\b\d{8}\b', 'ICO', True))
        
        # Addresses - more precise pattern
        patterns.append((r'\b[A-ZÁČĎÉĚÍŇÓŘŠŤÚŮÝŽ][a-záčďéěíňóřšťúůýž\s]{2,40}\s+\d{1,4}(?:/\d{1,4})?,\s*\d{3}\s?\d{2}\s+[A-ZÁČĎÉĚÍŇÓŘŠŤÚŮÝŽ][a-záčďéěíňóřšťúůýž\s]{2,30}(?=\s|$)', 'ADDRESS', False))
    ...
    def _has_relevant_context(self, context: str, category: str) -> bool:
        ...
        context_keywords = {
            ...
            'address': ['adresa', 'bydliště', 'ulice', 'sídlo'],
            'ico': ['ičo', 'ico', 'identifikační', 'číslo']
        }
...
class FixedAnonymizer:
    """Fixed anonymizer with improved detection"""
...
class DocumentProcessor:
    ...
    def __init__(self, anonymizer: FixedAnonymizer):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Fixed Czech Document Anonymizer',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python anonymizer_fixed.py document.docx
  python anonymizer_fixed.py document.txt --level full --output anonymized.txt
  python anonymizer_fixed.py data.docx --level standard --log-level DEBUG
        """
    )
    ...
    try:
        ...
        anonymizer = FixedAnonymizer(level)
