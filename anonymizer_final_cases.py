# -*- coding: utf-8 -*-
"""
Final Czech Document Anonymizer with Case Handling
==================================================
A final anonymization system that correctly handles Czech names in different cases.

Usage:
    python anonymizer_final_cases.py input.docx [--output output.docx] [--level full]
"""
...
class FinalCzechNameDetector:
    """Final Czech name detection with proper case handling"""
...
class FinalCaseAnonymizer:
    """Final anonymizer with proper case handling"""
    
    def __init__(self, level: AnonymizationLevel = AnonymizationLevel.STANDARD):
        self.level = level
        self.name_detector = FinalCzechNameDetector()
...
class DocumentProcessor:
    ...
    def __init__(self, anonymizer: FinalCaseAnonymizer):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Final Czech Document Anonymizer with Case Handling',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python anonymizer_final_cases.py document.docx
  python anonymizer_final_cases.py document.txt --level full --output anonymized.txt
  python anonymizer_final_cases.py data.docx --level standard --log-level DEBUG
        """
    )
    ...
    try:
        ...
        print(f"🔧 Inicializace finálního anonymizátoru s úrovní: {level.value}")
        
        anonymizer = FinalCaseAnonymizer(level)
